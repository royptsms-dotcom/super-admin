...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const namaHariRaya = null;

    if (dry_run) {
      return res.json({ jenis_standby: jenisStandby, nama_hari_raya: namaHariRaya });
    }

    const configKey = jenisStandby === 'hari_raya' ? 'harga_standby_hari_raya' : 'harga_standby_minggu';
    const { data: cfg } = await supabase.from('config').select('value').eq('key', configKey).single();
    const harga = parseFloat(cfg?.value || 0);

    const { data: standby, error } = await supabase
      .from('standby')
      .insert({ user_id: req.user.id, tanggal, jenis_standby: jenisStandby, nama_hari_raya: namaHariRaya, harga })
      .select().single();
    ...
    // Ambil grup WA berdasarkan job user
    ...
    await supabase.from('standby').update({ status_wa: waResult.success ? 'sent' : 'failed' }).eq('id', standby.id);
    await supabase.from('wa_log').insert({
      tipe_event: 'standby', ref_id: standby.id,
